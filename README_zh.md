...
**CLI 模式永久免费** — `pip install revhive-ai`，自带 Key，本地或 CI 里跑。按 BSL 1.1 你甚至可以**免费自托管用于生产**（唯一限制：不能把它作为竞争性托管审查服务转售）。如果自托管是你的路径——请给仓库点个 Star，并告诉我们哪些企业功能会让你愿意付费，那就是路线图。
...
BSL 1.1 — 详见 [LICENSE](LICENSE)。2030-05-12 自动转为 Apache 2.0。允许生产环境自托管（见[价格](#价格--把账算明白)）；许可仅限制将 RevHive 作为竞争性托管审查服务转售。
